...
关联模块是 PRD Helper 防止“材料很多但 PRD 仍然漏项”的关键环节。它不重新解释来源，也不生成最终文档；它只回答一件事：每条结构化需求上下文是否已经连到可实现、可验收的对象。
...
标准链路语言必须保持一致，脚本、模板和文档都以 `scripts/lib/id_registry.py` 中的实体和关系定义为准。
...
```text
fact -> page/feature -> rule -> data
fact -> page/feature -> rule -> acceptance
question/conflict/assumption -> affected fact/page/feature/rule/data/acceptance
```

如果某条事实暂时无法进入完整链路，不能静默丢弃；必须在 `context-map.md` 中标为断链或待确认影响范围。
...
关联通过只代表链路完整，不代表最终文档已经生成。进入 Generate 前仍需保留所有待确认问题和推断标记。
