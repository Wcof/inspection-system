<template>
  ...
  >
    <a-form layout="vertical" :model="form">
      ...
      <a-form-item v-else-if="form.taskType === 'charging'" label="充电站" required>
      ...
      <a-form-item v-else label="停车点" required>
...
const form = reactive<TemporaryDispatchForm>({ name: '', taskType: 'inspection', robotId: '', scheduledAt: '', reason: '', targetPointId: '', targetPointIds: [], conflictStrategy: 'delay' })
...
function resetForm() { form.name=''; form.taskType='inspection'; form.robotId=''; form.scheduledAt=''; form.reason=''; form.targetPointId=''; form.targetPointIds=[]; form.conflictStrategy='delay' }
function handleCancel() { emit('update:visible', false) }
function handleSubmit() {
  if (!form.name || !form.robotId || !form.scheduledAt) return message.error('请完整填写任务名称、执行机器人、计划执行时间')
  if (form.taskType === 'inspection' && !form.targetPointIds?.length) return message.error('请至少选择一个巡检点')
  if (form.taskType !== 'inspection' && !form.targetPointId) return message.error('请选择目标点')
